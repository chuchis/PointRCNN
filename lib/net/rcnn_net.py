...
class RefineRCNNNet(nn.Module):
    ...
    def forward(self, input_data):
        ...
        if cfg.RCNN.REF_CONFIG.USE_PROPOSALS:
            ...
            features = torch.cat((features, prop_feat.transpose(1,2).contiguous().unsqueeze(-1)), dim=1)
        ...
        if cfg.RCNN.REF_CONFIG.USE_RPN_FEATS:
            ...
            features = torch.cat((features, rpn_feats.transpose(1,2).contiguous().unsqueeze(-1)), dim=1)
        # print(features.shape)
        # print(xyz.shape)
